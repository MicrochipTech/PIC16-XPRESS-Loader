; USB 512-Word DFU Bootloader for PIC16F1454 MSD XPRESS Loader
; 2016 Lucio Di Jasio
; NOTE: Remove incompatible fuse words from .hex before adding as loadable
;        into final image!
;
; derived from
...
; - the RST Button is pressed (RA5 pin is grounded) at power-up or reset,
...
; using the internal oscillator and 3x PLL. A RED LED (RC3) is turned on upon
; bootloader entry
;
...
;;; Configuration
	__config _CONFIG1, _FOSC_INTOSC & _WDTE_OFF & _PWRTE_OFF & _MCLRE_ON & _CP_OFF & _BOREN_ON & _IESO_OFF & _FCMEN_OFF
...
USB_VENDOR_ID		equ	0x04d8	; MCHP
USB_PRODUCT_ID		equ	0x0057	; MCHP serial port
...
ep0_read_in
	...
	movwf	FSR0L
	movlw	high  (DESCRIPTOR_ADR)| 0x80
	movwf	FSR0H
...
; We have a valid application? Check if the entry pin is grounded
	banksel	PORTA
	btfss	PORTA,RA5		; test RST, modified for Xpress 18345 board
...
DESCRIPTOR_ADR	equ BOOTLOADER_SIZE-ALL_DESCS_TOTAL_LEN
	org	DESCRIPTOR_ADR
DEVICE_DESCRIPTOR
