...
#define CFG_ADDRESS 0x300000 // pic18f25K42
#define UID_ADDRESS 0x200000 
...
void lvpWrite( void){
    ...
    if (row_address >= (CFG_ADDRESS >> 1)) {    // use the special cfg word sequence
        LVP_cfgWrite( &row, CFG_NUM);
    }
    if(row_address >= (UID_ADDRESS >> 1)){
        // Ignore for now..
    }
    else { // normal row programming sequence
        LVP_addressLoad( row_address << 1);
...
void packRow( uint32_t address, uint8_t *data, uint8_t data_count) {
    ...
    uint32_t new_row = (address & 0xffffffc0) >> 1;
